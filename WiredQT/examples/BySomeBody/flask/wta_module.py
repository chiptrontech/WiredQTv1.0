...
def ExecuteQuerry(db,sql):
	...
	return results
def TableChart(cols,rec):
	
	
	ret=""
	for a in cols:
		ret+="data.addColumn('string', '%s');\n" % (a)
	_rec=[]
	for a in rec:
		_rec.append(list(a))
	ret+='''
data.addRows(%s
	...
	return ret
class DataGridChart:
	def __init__(self,dbtype="sqllite"):
		self._dbtype=dbtype
	def DataSource(self,db,sql):
		...
		cols=self.getFields(db,sql)        
		
		rec=ExecuteQuerry(db,sql)

		datagridValue=""
		#import pdb;pdb.set_trace()

		for a in cols:
			datagridValue+="data.addColumn('string', '%s');\n" % (a)
		_rec=[]
		for a in rec:
			_rec.append(list(a))
		datagridValue+='''
...
def createWidget(self,prop,control,parent,event=[]):
	...
	if control=="VScrollbar" or control=="HScrollbar":
		...
		if prop["Var"]=="":
			prop["Var"]='None'
		exec('self._%s=Gtk.%s(group=%s,label="%s")' % (name,control,prop["Var"],prop["Text"]))
		exec("self.%s=forms(self._%s)" % (name,name))
