...
class clsIntellisense:
	...
	def showhelp(self,code,word,x,y):
		print(word,y)
		lst_underscore=[]
		def extractlst(lst,x,lastletter="x"):
			suggest=script.complete(y,x+1)
			def getparams(j):
				fn="description=\'param "
				ret=""
				for x in j.params:
					x=str(x)
					ret+=x[x.find(fn)+len(fn):-2]    
					ret+=","
				return "(" + ret[:-1] + ")"
			...
			try:
				...
				for i,sug in enumerate(suggest):
					...
					#if lastletter==".":break# dont entertain if lastletter=="."
					param=''
					stype=sug.type
					if sug.module_name=="__main__":
						print(stype,sug.name,sug.module_name,sug.module_path,sug.line)
					if stype=='module':
						continue
					if stype=='function':
						param=getparams(sug)
					...
					if len(sug_name)>0:	
						if sug_name[0]=="_":
							lst_underscore.append(sug_name+param)	
						else:	
							lst.append(sug_name+param)				
					
			except AssertionError:
				print("jedi parser warning occur!!!")

			lst=lst+lst_underscore									
					    
			lst+=[a.name for a in suggest]#bcoz it is pointer 

			modulenotinfolder=False
			index=modulename.find("module ")
			if index!=-1: 
				fname=modulename[index+len("module "):-3]
				if FileExist(fname+".py")==False:
					modulenotinfolder=True
			if modulename!="" and modulenotinfolder==True:#add only if module not in project directory eg numpy etc
				#self.jedimodule.update({modulename:lst})
				pass
		...
		sys_path =self.selfhandler.caller.jedi.get_default_environment().get_sys_path()
		path=GetPath(self.selfhandler.filename)[:-1]
		script=self.selfhandler.caller.jedi.Script(source=code,sys_path=sys_path + [path])
		cc=getvardefinition(word, x)
		lst=[]
		try: 	#fixme, x doesnt point to exact cursor location like eg x.available where cursor is in between i and l
			if cc[1] !=-1:
				modulename=str(script.infer(y,cc[1]))
				if modulename!='[]':
					if (modulename in self.jedimodule)==False:
						try:
							extractlst(lst,len(word)+1,word[-1])
						except:
							pass
					else:
						lst=self.jedimodule[modulename]
			else:
				modulename=""
				extractlst(lst,x-1)	
		except:
	...
	def words(self):
		...
		for i,a in enumerate(linestr):
			...
			if num==self.selfhandler.xIntel:
				break
		print(i)   
		word=linestr[:i+2].strip()
		nword=word
		x=i+1
		#word,_,_=self.ShowTypedWord(linestr,x-2)
		...
		ret,xword=self.showhelp(code,word, x,y)
		self.posX=x
		self.posY=y-1
	...
	def updatevalue(self,fromlabel=None):
		...
		try:
			...
			if self.lastselected=="Picture":
				strs1='forms(self.widget.name).LoadPicture="'+ value + '"'
				exec(strs1)
			if self.widget.prop!="":
				prop=eval(self.widget.prop)
				prop=eval(self.widget.prop)
...
class Handler(QtWidgets.QWidget,usercontrol):
	def __init__(self,*param):    
		...
		self.gui=ScrollLayout(self,500,0,200,700)
	...
	def toolbar(self):
		...
		_createwidget('QPushButton','resume',120,t,30,30)
		...
		self.QColorDialog.BackColor=[0.2,0.2,0.2,1]
		...
		_createwidget('QsciScintilla','Entry1',self.thisleft,550,self.thiswidth,40)

		lexer = QsciLexerPython()
		#lexer.setDefaultFont(font)
		self.Entry1.setLexer(lexer)
		text = bytearray(str.encode("Arial"))   
		self.Entry1.obj.SendScintilla(QsciScintilla.SCI_STYLESETFONT, 1)        

		self.Entry1.obj.linesChanged.connect(self.linesChanged)
		self.Entry1.obj.SendScintilla(QsciScintilla.SCI_SETHSCROLLBAR, 0)
		self.Entry1.obj.SendScintilla(QsciScintilla.SCI_SETVSCROLLBAR, 0)
	...
	def linesChanged(self):
		if self.Entry1.Text=='':return
		if self.caller.current_debug==None: 
			self.Entry1_clear=True
			return
		self.caller.current_debug.execPDB(self.Entry1.Text.replace("\r\n",""))
		self.Entry1_clear=True#for this Entry1
	...
	def QSCIShow(self,flag):
		...
		forms(self.QSCI).Top=forms(self.sa).Top
		forms(self.QSCI).Left=forms(self.sa).Left
		forms(self.QSCI).Width=forms(self.sa).Width
		forms(self.QSCI).Height=forms(self.sa).Height
			...
			for a in xline: 
				if a.find('self.createwidget(')!=-1 and a.find('self.createwidget')!=-1 :
					...
					try:
						...
						exec(a)#createWidget
						if FileExist(GetPath(self.filename)+param['Picture']):
						...
						#print("invalid self.createwidget in " + fname)
						QMessageBox.about(self, "Error", "Check " + fname + ", if required Libraries were installed")
	...
	def breakpoint_clicked(self,flag):
		cmd='b '+str(self.QSCI.obj.getCursorPosition()[0]+1)+'\n'
		#self.showdebugbutton(False)
		self.process.stdin.write(cmd.encode())
		self.process.stdin.flush()
		pass
	...
	def save(self):
		...
		if self.IsQTForms():
			self.GenerateWidget()
		SaveFileStr(self.filename, self.QSCI.Text)	
	def saveas(self,fname):
		self.code=self.QSCI.Text
		if self.IsQTForms():
			self.GenerateWidget()
		self.filename=fname
		SaveFileStr(fname, self.QSCI.Text)			
	...
	def intellisense_activated(self,e):
		...
		if hasattr(self,'fromcodeIntel') and self.fromcodeIntel:
			...
			self.QSCI.obj.insertAt(strs,self.clsintellisense.posY,self.clsintellisense.posX)
			self.QSCI.setCursorPosition(self.clsintellisense.posY,self.clsintellisense.posX+len(strs))
	...
	def GTKForms(self):
		self.createwidget("{'Enable': 'True', 'Height': '25', 'Events': '[activated]', 'Top': '57', 'Width': '200', 'BackColor': '[1.0, 1.0, 1.0, 1.0]', 'Name': 'intellisense', 'Picture': '', 'ForeColor': '[0.0, 0.0, 0.0, 1.0]', 'Left': '356', 'ParentsType': '', 'Tag': '', 'Help': '', 'Var': '', 'Font': 'None', 'Text': '', 'Visible': 'False'}",'QComboBox','usercontrol',"[['activated', 'self,e']]")
	def Widget(self):
		return self  
	...
	def CheckTraceError(self):
		...
		if line!='':
			if line.find('Traceback')!=-1 or line.find('SyntaxError')!=-1:
				#self.Entry1.Enable=True
				self.process.terminate()
				self.OutputPane.Text+=line
				self.OutputPane.obj.setCursorPosition(self.OutputPane.obj.lines(),1)
				self.caller.updateAllOutputPane()
				QMessageBox.about(self, "WiredQT, Error Detected!!!",self.OutputPane.Text)
				...
				print(line,"raks")    
	def loop(self):
		if hasattr(self,'Entry1_clear') and self.Entry1_clear:
			self.Entry1_clear=False
			self.Entry1.Text=""		
		if self.process!=None:
	...
	def eventFilter(self, obj, event):
		...
		if event.type() == QtCore.QEvent.KeyPress:
			...
			if self.objectMove.obj!=None and event.key()==16777223:
				lst=[]
		...
		return a
	def SourceBrowse(self,cls=""):
		sys_path =self.caller.jedi.get_default_environment().get_sys_path()
		path=GetPath(self.filename)[:-1]
		#script=self.caller.jedi.Script(source=self.QSCI.Text,sys_path=sys_path + [path])
		#suggest=script.complete(self.QSCI.obj.lines(),0)
		_lst=[]
		script=self.caller.jedi.Script(source=self.QSCI.Text+cls,sys_path=sys_path + [path])
		suggest=script.complete(self.QSCI.obj.lines(),len(cls))
		for sug in suggest:
			if sug.module_name=="__main__" and (sug.type=="class" or sug.type=="function"):
				_lst.append([sug.type,sug.name,sug.module_name,sug.line])
		return _lst	
