...
def createWidget(self,prop,control,parent,event=[]):
	...
	if control=="VScrollbar" or control=="HScrollbar":
		...
		exec("self.%s=forms(self._%s)" % (name,name))
	dontaltertextandcolor=["QScrollArea","QWidget","VScrollbar","HScrollbar","SpinButton","ScrolledWindow"]	
	...
	if prop['ParentsType']=="Layout" and prop['Tag']!='Activex':
		cmd="self._%s.put(self._%s,%s,%s)"%(parent,name,prop["Left"],prop["Top"])
	elif prop['ParentsType'] in ["QWidget","QGroupBox","QFrame"]:
	...
	if control=="Image":
		...
		if parent=="usercontrol":
			cmd="self._%s.put(self._%sEvent,%s,%s)"%(parent,name,prop["Left"],prop["Top"])
		else:
			cmd="self._%s.put(self._%sEvent,%s,%s)"%(parent,name,float(prop["Left"]),float(prop["Top"]))
		eval(cmd)
	if control=="Entry":
...
#			self.obj.set_from_file (fname)	
	...
	def ForeColor(self):		#value=0.0, 1.0, 0.0, 1.0 #RGBA
		...
		style=''
		for b in a:
			if b.find('color')!=1 and b.find('background')==-1:
