...
def wtaprop():
	wtaprop=eval("{'Name': 'Label1', 'Var': '', 'Font': '', 'Enable': 'True', 'Top': '0', 'Width': '80', 'ParentsType': '', 'Picture': '', 'ForeColor': '(0,0,0,1)', 'Visible': 'True', 'BackColor': '(1,1,1,0.25)', 'Events': '[]', 'Height': '30', 'Left': '0', 'Tag': '', 'Text': 'Label1', 'Help': ''}")
...
def createWidget(self,prop,control,parent,event=[]):
	...
	if prop['Tag']=='Activex':
		if self.caller!=None and control.find("/")==-1 and hasattr(self.caller,'filename'):
			dst=GetPath(self.caller.filename)+control+".py"
			if FileExist(dst)==False:
				#check plugin and copy to roject folder
				plugin=app_path().replace('\\',"/")+"/plugin/"+control+"/"+control+".py"
				if FileExist(plugin):
					import shutil
					shutil.copyfile(plugin, dst)
			control=GetPath(self.caller.filename)+control	+".py"	
	...
	if prop['ParentsType'] in ["QWidget","QGroupBox","QFrame","QScrollArea"]:
		if prop['ParentsType']=="QScrollArea":
			cmd="%s.setWidget(self.%s.obj)"%(parent,name)	
		else:	
...
def setPicture (imgGTK,imgCV2):
	...
	if channel==3:
		qImg = QImage(imgCV2.data, width, height, bytesPerLine, QImage.Format_BGR888)
	if channel==1:
